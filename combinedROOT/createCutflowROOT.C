...
void doThings(std::string inFileName, std::string outFileName, double &eventScaleFactors, std::string year, std::string systematic, std::string dataBlock)
{
   ...
   for(auto systematic_suffix = systematic_suffices.begin(); systematic_suffix < systematic_suffices.end();systematic_suffix++)
   {
      ...
      TH1F *h_nDijet_pairs_semiRAW= new TH1F("h_nDijet_pairs_semiRAW","Heavy Dijet Pairs / Event (H_{T}, nAK8 cuts); Number of Dijet Pairs (M_{dijet} > 1 TeV); Events",6,-0.5,5.5);
      ...
      TH1F *h_AK8_pt= new TH1F("h_AK8_pt", "AK8 jet p_{T} (H_{T} > 1.5 TeV); AK8 jet p_{T}; Events / 50 GeV",100,0.,5000);
      TH1F *h_AK8_eta= new TH1F("h_AK8_eta", "AK8 jet eta (H_{T} > 1.5 TeV); AK8 jet eta; Events / 0.1",50,-2.5,2.5);
      TH1F *h_AK8_phi= new TH1F("h_AK8_phi", "AK8 jet phi (H_{T} > 1.5 TeV); AK8 jet phi; Events / 0.1 rads",80,-4.,4.);

      TH1F *h_AK4_pt= new TH1F("h_AK4_pt", "AK4 jet p_{T} (H_{T} > 1.5 TeV); AK4 jet p_{T}; Events / 50 GeV",100,0.,5000);
      TH1F *h_AK4_eta= new TH1F("h_AK4_eta", "AK4 jet eta (H_{T} > 1.5 TeV); AK4 jet eta; Events / 0.1",50,-2.5,2.5);
      TH1F *h_AK4_phi= new TH1F("h_AK4_phi", "AK4 jet phi (H_{T} > 1.5 TeV); AK4 jet phi; Events / 0.2 rads",80,-4.0,4.0);
   ...
   for(auto datayear = years.begin();datayear<years.end();datayear++)
   {
      ...
      for(auto dataBlock = dataBlocks.begin();dataBlock < dataBlocks.end();dataBlock++)
      {
         for( auto systematic = systematics.begin(); systematic < systematics.end();systematic++)
         {
            ...
            std::string outFileName = (*dataBlock+year +  "_"+ systematic_str+ "_CUTFLOW.root").c_str();
