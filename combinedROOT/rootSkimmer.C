...
// rootSkimmer.C
// takes combined root files stored on eos (/store/user/ecannaer/skimmedFiles/)
// and applies the initial selection 
...
bool doThings(std::string inFileName, std::string outFileName, double &eventScaleFactors, std::string year, std::vector<std::string> systematics, std::string dataBlock, std::string runType, bool dryRun)
{
   ...
   double total_events_unscaled = 0, prefiringWeight = 1.0;
   ...
   */
   ...
   for(auto systematic_ = systematics.begin();systematic_ < systematics.end();systematic_++)
   {
      ...
      for(auto systematic_suffix = systematic_suffices.begin(); systematic_suffix < systematic_suffices.end();systematic_suffix++)
      {
         ...
         double nPassedAK8VetoMap = 0, nPassedAK8HEM = 0, nPassedLepton = 0, nPassedTrigger = 0;
         ...
         bool isNom = (systematic == "nom");   // used to keep all nom events for plotting purposes
         ...
         {  
            ...
            // (C): Jet veto map over AK8 jets
            // (D): HEM veto map over AK8 jets
            // (E): HT cut (> 1600 GeV for mainband)
            // (F): nAK8 jet cut (nAK8 > 2)
            // (G): nHeavyAK8 

            bool passedPreviousCuts = true;

            //////////// START initial-initial selection: apply the triggers, lepton filters, jet veto map for AK8, and HEM for AK8

            ///// (A) -------> APPLY TRIGGER 
            if ((!passesPFHT) && (!passesPFJet) )
            {
               nFailed_triggers++;
               if(!isNom)continue; // skip events that don't pass both triggers
               else{  passedPreviousCuts = false;}
            } 
            if(passedPreviousCuts)nPassedTrigger+=eventWeight;
            ...
            if(passedPreviousCuts)nPassedLepton+=eventWeight;
            ...
            if(passedPreviousCuts)nPassedAK8VetoMap+=eventWeight;
            ...
            if(passedPreviousCuts)totEventsUncut+=eventWeight;


            //////////// END initial-initial selection



            if(passedPreviousCuts)nPassedAK8HEM+=eventWeight;

            if(passedPreviousCuts) nEvents+=eventWeight;
            ...
            if(passedPreviousCuts)nHTcut+=eventWeight;
            ...
            if(passedPreviousCuts)nAK8JetCut+=eventWeight;
            ...
            if(passedPreviousCuts)nHeavyAK8Cut+=eventWeight;
            ...
            bool goodToFill = ((!isNom) || ( isNom && passedPreviousCuts ));   // = true if not nom, true if (nom && passedPreviousCuts)
         ...
         if(verbose)
         {
            ...
            std::cout << "More_Preselect_Info:   Signal  Region for " << systematic+ "_" + *systematic_suffix << " " << dataBlock << ", "<<  year << " and " << tree_string <<   ": nPassedTrigger/nPassedLepton/nPassedAK8VetoMap/nPassedAK8HEM: " << nPassedTrigger<<"-" << nPassedLepton<<"-" <<nPassedAK8VetoMap<<"-" <<nPassedAK8HEM << std::endl;
            std::cout << "More_Preselect_Info:   Control  Region for " << systematic+ "_" + *systematic_suffix << " " << dataBlock << ", "<<  year << " and " << tree_string <<   ": nPassedTrigger/nPassedLepton/nPassedAK8VetoMap/nPassedAK8HEM: " << nPassedTrigger<<"-" << nPassedLepton<<"-" <<nPassedAK8VetoMap<<"-" <<nPassedAK8HEM << std::endl;
            std::cout << "More_Preselect_Info:   AT1b  Region for " << systematic+ "_" + *systematic_suffix << " " << dataBlock << ", "<<  year << " and " << tree_string <<   ": nPassedTrigger/nPassedLepton/nPassedAK8VetoMap/nPassedAK8HEM: " << nPassedTrigger<<"-" << nPassedLepton<<"-" <<nPassedAK8VetoMap<<"-" <<nPassedAK8HEM << std::endl;
            std::cout << "More_Preselect_Info:   AT0b  Region for " << systematic+ "_" + *systematic_suffix << " " << dataBlock << ", "<<  year << " and " << tree_string <<   ": nPassedTrigger/nPassedLepton/nPassedAK8VetoMap/nPassedAK8HEM: " << nPassedTrigger<<"-" << nPassedLepton<<"-" <<nPassedAK8VetoMap<<"-" <<nPassedAK8HEM << std::endl;
         }

         ////////
         std::cout << "passed/total= : " << (1.0*passedEvents)/totalEvents <<   "----------- passedEvents: " << passedEvents << ", totalEvents: " << totalEvents << ",  nFailed_triggers/nFailed_leptons/nFailed_veto_maps/nFailed_hem/nFailed_HT/nFailed_nAK8/nFailed_nHeavyAK8: " <<nFailed_triggers << "/" <<nFailed_leptons << "/" << nFailed_veto_maps<< "/" <<nFailed_hem << "/" << nFailed_HT<< "/" << nFailed_nAK8<< "/" <<nFailed_nHeavyAK8 << std::endl;
      }                    
...
"Suu8_chi2_ZTZT_","Suu8_chi2p5_ZTZT_","Suu8_chi3_ZTZT_"};  
   ...
   bool dryRun        = false;   // run without creating a file (testing)
