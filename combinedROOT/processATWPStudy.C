...
bool doThings(std::string inFileName, std::string outFileName, double& nEvents, double& nHTcut, double& nAK8JetCut,double& nHeavyAK8Cut, double& nBtagCut, double& nDoubleTagged,double& nNoBjets, double& nDoubleTaggedCR, double& NNDoubleTag, double& nZeroBtagAntiTag, double & nOneBtagAntiTag, double & nNN_SR, double & nNN_CR, double & nNN_AT1b, double & nNN_AT0b,  std::string dataYear,std::string systematic, std::string dataBlock, double BEST_WP, double BEST_AT_WP, bool verbose = false)
{
   ...
   for(auto systematic_suffix = systematic_suffices.begin(); systematic_suffix < systematic_suffices.end();systematic_suffix++)
   {
		...
		TH1F* h_BEST_score_ATSJ_AT1b  = new TH1F("h_BEST_score_ATSJ_AT1b","SJ BEST Score (SJ1 AT) (1b region); SJ2 NN Score; superjets",50,0,1.0);
		TH1F* h_BEST_score_ATSJ_AT0b  = new TH1F("h_BEST_score_ATSJ_AT0b","SJ BEST Score (SJ1 AT) (0b region); SJ2 NN Score; superjets",50,0,1.0);
		TH1F* h_BEST_score_CR    = new TH1F("h_BEST_score_CR",  "SJ BEST Score (CR); SJ NN Score; superjets",50,0,1.0);

		TH1F* h_BEST_score_ADT0b  = new TH1F("h_BEST_score_ADT0b","SJ BEST Score (ADT0b);NN Score; superjets",50,0,1.0);
		TH1F* h_BEST_score_ADT1b  = new TH1F("h_BEST_score_ADT1b","SJ BEST Score (ADT1b);NN Score; superjets",50,0,1.0);



		TH1F* h_BEST_score_TSJ_0b    = new TH1F("h_BEST_score_TSJ_0b",  "SJ2 BEST Score (Tagged SJ1) (0b Region); NN Score; superjets",50,0,1.0);
		TH1F* h_BEST_score_TSJ_1b    = new TH1F("h_BEST_score_TSJ_1b",  "SJ2 BEST Score (Tagged SJ1) (1b Region); NN Score; superjets",50,0,1.0);
		...
		{  
		  ...
		  if ( (!passesPFHT) && (!passesPFJet) ) 
		  {
			...
			if( nMedBTags < 1 ) 
			{
				...
				{
					double eventWeightToUse_ST = eventScaleFactor * pdf_weight*scale_weight * bTag_eventWeight_M;
					h_BEST_score_TSJ_0b->Fill(SJ2_BEST_scores,eventWeightToUse_ST);
				}
				else if ( SJ2_BEST_scores > BEST_WP)
				{
					double eventWeightToUse_ST = eventScaleFactor * pdf_weight*scale_weight * bTag_eventWeight_M;
					h_BEST_score_TSJ_0b->Fill(SJ1_BEST_scores,eventWeightToUse_ST);
				}
				...
				if( (SJ1_BEST_scores > BEST_WP) && (SJ2_BEST_scores > BEST_WP) ) 
				{
					...
					h_BEST_score_CR->Fill(SJ1_BEST_scores,eventWeightToUse_NN);
					h_BEST_score_CR->Fill(SJ2_BEST_scores,eventWeightToUse_NN);
				...
				else if(  (SJ1_BEST_scores < BEST_AT_WP) && (SJ2_BEST_scores < BEST_AT_WP) ) // want to be quite sure the second superjet is not tagged
				{
					...
					h_BEST_score_ADT0b->Fill(SJ1_BEST_scores, eventWeightToUse_NN);
					h_BEST_score_ADT0b->Fill(SJ2_BEST_scores, eventWeightToUse_NN);
			...
			else if ( (nMedBTags > 0)  )
			{
				...
				{
					double eventWeightToUse_ST = eventScaleFactor * pdf_weight*scale_weight * bTag_eventWeight_M;
					h_BEST_score_TSJ_1b->Fill(SJ2_BEST_scores,eventWeightToUse_ST);
				}
				else if ( SJ2_BEST_scores > BEST_WP)
				{
					double eventWeightToUse_ST = eventScaleFactor * pdf_weight*scale_weight * bTag_eventWeight_M;
					h_BEST_score_TSJ_1b->Fill(SJ1_BEST_scores,eventWeightToUse_ST);
				}
				...
				else if(  (SJ1_BEST_scores < BEST_AT_WP) && (SJ2_BEST_scores < BEST_AT_WP) ) // want to be quite sure the second superjet is not tagged
				{
					...
					h_BEST_score_ADT1b->Fill(SJ1_BEST_scores, eventWeightToUse_NN);
					h_BEST_score_ADT1b->Fill(SJ2_BEST_scores, eventWeightToUse_NN);
				}  
...
{  
   ...
   systematics = {"nom", "scale", "bTagSF_med",  "JER", "JEC", "PUSF", "pdf"};
   std::vector<double> WPs = {0.05,0.10,0.125,0.15,0.175,0.20}; // ,0.92,0.95,0.97,0.98,0.99
   ...
   for(auto WP = WPs.begin(); WP < WPs.end(); WP++)
   {
   	...
   	std::string outputFolder = "ATWP_study/WP" + WPDoubleStr + "/";
	   ...
	   for(auto dataYear = dataYears.begin(); dataYear < dataYears.end();dataYear++ )
	   {
			...
			{
				dataBlocks = {"TTToHadronicMC_", "TTToSemiLeptonicMC_" , "TTToLeptonicMC_",
			...
			for(auto systematic = systematics.begin();systematic<systematics.end();systematic++)
			{
				...
				for(auto dataBlock = dataBlocks.begin();dataBlock < dataBlocks.end();dataBlock++)
				{
					...
					std::string outFileName = (outputFolder  + *dataBlock+ year + "_ATWP" + WPDoubleStr + "_processed.root").c_str();
					...
					if (!doThings(inFileName,outFileName,nEvents,nHTcut,nAK8JetCut,nHeavyAK8Cut,nBtagCut,nDoubleTagged,nNoBjets,nDoubleTaggedCR, NNDoubleTag,nZeroBtagAntiTag, nOneBtagAntiTag, nNN_SR, nNN_CR, nNN_AT1b, nNN_AT0b, *dataYear,*systematic, *dataBlock, BEST_WP, *WP, _verbose ))
					{
