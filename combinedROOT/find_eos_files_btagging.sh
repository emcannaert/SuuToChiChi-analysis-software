...
then 
	...
	xrdfs root://cmseos.fnal.gov ls -R /store/user/ecannaer/$1 > all_files_btag.txt
	...
	grep -v root all_files_btag.txt | grep $2 | grep "MSuu" | grep /000  > signal_eos_paths_btagging.txt
	grep -v root all_files_btag.txt | grep $2 | grep "TTTo" | grep /000  > TTbar_eos_paths_btagging.txt
	grep -v root all_files_btag.txt | grep $2 | grep "QCD" | grep /000  > QCD_eos_paths_btagging.txt
	grep -v root all_files_btag.txt | grep $2 | grep "ST_" | grep /000  > ST_eos_paths_btagging.txt	
	grep -v root all_files_btag.txt | grep $2 | grep "data" | grep /000  > data_eos_paths_btagging.txt
	echo "Finished."
    #rm all_files.txt
fi
