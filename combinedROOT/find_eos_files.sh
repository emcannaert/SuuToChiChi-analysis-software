...
then 
	...
	grep -v root all_files.txt | grep -E "_${2}_"  | grep "QCD_HT" | grep /000  > QCD_eos_paths.txt
	grep -v root all_files.txt | grep -E "_${2}_"  | grep "QCD_Pt" | grep /000  > QCD_Pt_eos_paths.txt
