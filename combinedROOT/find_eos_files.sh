...
then 
	...
	grep -v root all_files.txt | grep "_$2" | grep "QCD" | grep /000  > QCD_eos_paths.txt
	grep -v root all_files.txt | grep "_$2" | grep "ST_" | grep /000  > ST_eos_paths.txt	
	grep -v root all_files.txt | grep "_$2" | grep "data" | grep /000  > data_eos_paths.txt
	echo "Finished."
    #rm all_files.txt
fi
