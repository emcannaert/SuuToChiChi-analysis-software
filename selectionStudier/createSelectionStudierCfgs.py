...
def makeACfg(sample, year, systematic__, datafile, all_systematics):
   ...
   if isSignal:
      if systematic__ == "JEC":
         newCfg = open("%s/selectionStudier_%s_%s_%s_cfg.py"%(output_dir,sample,year,systematic__),"w")
      else: 
         newCfg = open("%s/selectionStudier_%s_%s_cfg.py"%(output_dir,sample,year),"w")
   elif systematic__ == "":
      newCfg = open("%s/selectionStudier_%s_%s_%scfg.py"%(output_dir,sample,year, systematic__),"w")
   ...
   for systematic in systematic_:
      ...
      for iii, suffix in enumerate(systematicSuffices):
         ...
         newCfg.write('process.selectionStudier_%s%s = cms.EDAnalyzer("selectionStudier",\n'%(systematic, suffix) )
   ...
   if "Suu" in sample:
      ...
      if systematic__ == "nom":
         use_syst = "nom"
      newCfg.write('process.TFileService = cms.Service("TFileService",fileName = cms.string("selectionStudier_%s_%s_%s_output.root")\n'%(sample,year,use_syst))
   ...
   for systematic in systematic_:
      ...
      for iii, suffix in enumerate(systematicSuffices):
         ...
         newCfg.write(" * process.selectionStudier_%s%s\n"%(systematic, suffix) )
   newCfg.write(")\n")
         ...
         for systematic in systematics:
            ...
            if "Suu" in sample:
               try:
                  ...
                  makeACfg(sample, year, systematic, signal_files[year][sample], systematics)   # change input to write systematic type
                  num_files+=1
               except:
                  
                  print("Failed on sample/year/systematic: %s/%s/%s"%(sample,year,systematic))
            else:
               makeACfg(sample, year, systematic, datafiles[year][sample], systematics)  # change input to write systematic type
               num_files+=1 
