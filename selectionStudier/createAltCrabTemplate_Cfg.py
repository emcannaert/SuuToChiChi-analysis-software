...
def makeAltCrabCfg(sample, year, systematic, dataset,dateTimeString):
	...
	if "Suu" in sample:
		...
		path_backtrack = "../"
	if systematic == "nom":
		newCfg = open("allAltCrabCfgs/%scrab_selectionStudier_%s_%s_cfg.py"%(file_base,sample,year),"w")
	else:
		newCfg = open("allAltCrabCfgs/%scrab_selectionStudier_%s_%s_%s_cfg.py"%(file_base,sample,year, systematic),"w")
   
	newCfg.write("from CRABClient.UserUtilities import config\n")
	newCfg.write("config = config()\n")
	newCfg.write("config.General.requestName = 'selectionStudier_%s_%s_%s_AltDatasets_000'\n"%(sample,year,systematic))
	...
	if "data" in sample:
		...
		#else: newCfg.write("config.JobType.maxMemoryMB = 00 # might be necessary for some of the QCD jobs\n")
	...
	newCfg.write("config.Data.outputDatasetTag = 'selectionStudier_%s_%s_%s'\n"%(sample,year,systematic))
	newCfg.write("config.Data.outLFNDirBase = '/store/user/ecannaer/selectionStudy_%s%s'\n"%(dateTimeString,sideband_str))
