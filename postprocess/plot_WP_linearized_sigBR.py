...
def plot_linearized_signal_vs_BR_histogram(year,region,mass_point, technique_type, WP):
    ...
    file_name = "ATWP_study/finalCombineFilesNewStats/%s/combine_%s%s_%s.root"%(WP_folder,technique_type, year, mass_point)
    ...
    output_file = "plots/linearized_sigBR/ATWP_study/sig_vs_BR_%s_%s%s_%s_%s.png"%(mass_point,technique_type,region,year,WP)
    c.SaveAs(output_file)

    sig_hist.Draw("HIST")
    output_file = "plots/linearized_sig/ATWP_study/linearized_sig_%s_%s%s_%s_%s.png"%(mass_point,technique_type,region,year,WP)
    c.SaveAs(output_file)
...
def plot_linearized_signal_vs_BR_multi_mass(year,region, technique_type, WP):
    ...
    file_names = [ "ATWP_study/finalCombineFilesNewStats/%s/combine_%s%s_%s.root"%(WP_folder,technique_type, year, mass_point) for mass_point in mass_points ] 
    ...
    output_file = "plots/linearized_sigBR/ATWP_study/sig_vs_BR_multi_mass_%s%s_%s_%s.png"%(technique_type,region,year,WP)
    c.SaveAs(output_file)
...
if __name__=="__main__":
    ...
    regions = ["AT0b", "AT1b"]   # "AT1tb", "AT0tb"
    technique_types = ["NN_"]

    if debug:
        years = ["2015"]
        mass_points = ["Suu4_chi1",] 
        regions = ["AT1b"]   # "AT1tb", "AT0tb"
        technique_types = [""]

    WPs = [0.05,0.10,0.12,0.15,0.17,0.20,0.25]
        ...
        for WP in WPs:
            ...
            for region in regions:
                for technique_type in technique_types:
                    ...
                    plot_linearized_signal_vs_BR_multi_mass(year,region, technique_type,WP_str) ## only need to run once per year/region/technique
                    ...
                    #for mass_point in mass_points:
                        ...
                        #plot_linearized_signal_vs_BR_histogram(year,region,mass_point, technique_type, WP_str)
                        #run_count+=1
