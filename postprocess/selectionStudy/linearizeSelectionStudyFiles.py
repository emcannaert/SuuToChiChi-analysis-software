...
class linearized_plot:
	...
	def __init__(self, year, mass_point, technique_str, all_BR_hists,output_map_file,  use_QCD_Pt=False, useMask=False, use_1b_bin_maps = False, createDummyChannel=False, run_from_eos = False, runCorrected = False, WP=None ,debug=False):
		...
		if "ET" in self.WP:
			self.MC_root_file_home	  = 	os.getenv('CMSSW_BASE') + "/src/combinedROOT/selectionStudy/%s/"%WP
			self.data_root_file_home	=   os.getenv('CMSSW_BASE') + "/src/combinedROOT/selectionStudy/%s/"%WP
		...
		if "ET" in self.WP:
			self.final_hist_name = "h_MSJ_mass_vs_MdSJ_postbTag"
		...
		if "ET" in self.WP:
			self.output_file_home	= os.getenv('CMSSW_BASE') + "/src/postprocess/selectionStudy/%s/finalCombineFilesNewStats/%s/"%(self.WP,self.use_QCD_Pt_str)
			self.final_plot_home	 = os.getenv('CMSSW_BASE') + "/src/postprocess/plots/selectionStudy/%s/finalCombinePlots/%s/"%(self.WP,self.use_QCD_Pt_str)
...
if __name__=="__main__":
   ...
   "Suu7_chi1p5","Suu7_chi2","Suu7_chi2p5","Suu7_chi3","Suu8_chi1","Suu8_chi1p5","Suu8_chi2","Suu8_chi2p5","Suu8_chi3"]
	...
	use_QCD_Pt_opts = [False]
	use_QCD_Pt_strs = ["QCDHT"]
		...
		for WP in WPs:
			...
			for jjj,use_QCD_Pt in enumerate(use_QCD_Pt_opts):
				...
				for iii,technique_str in enumerate(technique_strs):
					...
					all_BR_hists  = hist_loader(year, technique_str, use_QCD_Pt, doHTdist, doSideband, doATxtb, includeTTJets800to1200, includeTTTo, includeWJets, run_from_eos, runCorrected, WP)

					for mass_point in mass_points:
						#try:

						print("Running for %s/%s/%s/useQCDPT = %s"%(year,mass_point,technique_descr[iii],use_QCD_Pt_strs[jjj] ))
						if usMask: final_plot = linearized_plot(year, mass_point, technique_str, all_BR_hists, output_map_file,use_QCD_Pt, True, use_1b_bin_maps, createDummyChannel,run_from_eos, runCorrected,WP, debug)   ### run with masked bins
						else: final_plot = linearized_plot(year, mass_point, technique_str, all_BR_hists, output_map_file, use_QCD_Pt, False, use_1b_bin_maps ,createDummyChannel,run_from_eos, runCorrected, WP, debug)	### run without masked bins
