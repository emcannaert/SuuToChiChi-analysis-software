...
import pdb
import time
from histInfoSelectionStudy import histInfoSelectionStudy
...
class combineHistBins:
	...
	def __init__(self,year,technique_str, hist_name, debug=False, dryRun=False, runEos=False,useQCDHT=False, WP=None, max_stat_uncert = 0.175):   #constructor will convert TH2 to a 2D array, do the optimized bin merging, and then return a TH2 with those associated bins
		...
		self.min_unscaled_QCD_bin_counts   = 1.0   ## the minimum number of unscaled QCD events required to be in each bin, better to make this 1 or more to prevent weird migration stuff
		self.min_scaled_QCD_bin_counts   = 1.0   ## the minimum number of unscaled QCD events required to be in each bin, better to make this 1 or more to prevent weird migration stuff
		...
		self.all_hist_values =  histInfoSelectionStudy.histInfoSelectionStudy(year, hist_name, self.bin_min_x, self.bin_min_y, self.n_bins_x, self.n_bins_y, self.technique_str, self.includeTTJetsMCHT800to1200, self.includeWJets, self.includeTTo, self.useQCDHT, debug, runEos, self.WP)   
	...
	def sort_bins_by_descending_event_yield(self): 
		...
		output_path = 'plots/statUncertaintyPlots/postMergedPlots/selectionStudy/%s/'%(self.WP)
		if not os.path.exists(output_path):
			os.makedirs(output_path)
		plt.savefig(text_output_path + "%s_post_merge_superbin_yields_%s.png"%(self.QCD_type, self.year))
		plt.close()
	...
	def print_summary(self): # check the total integral of the PRE-merged distribution and the post-merged, linearized distribution to make sure they are the same
		...
		print("")
		print("")
		print( "--------- Merged N_x/N_y = %s/%s original bins --- for year --- %s --- and --- %s --- technique into %s linearized bins. "%(self.n_bins_x,self.n_bins_y, self.year, technique_title, len(self.superbin_indices) ))
...
if __name__=="__main__":
	...
	hist_names = ["h_MSJ_mass_vs_MdSJ_postSJTag","h_MSJ_mass_vs_MdSJ_postbTag"]
	output_strs = ["",""]
	technique_strs = ["cut-based","cut-based"]
	...
	if debug:
		...
		hist_names = ["h_MSJ_mass_vs_MdSJ_postbTag"]
	...
	for WP in WPs:
		...
		for useQCDHT_opt in useQCDHT_opts:

			if useQCDHT_opt: QCD_type = "QCDHT"
			else: QCD_type = "QCDPT"

			text_output_path  = os.getenv('CMSSW_BASE') + "/src/SuuToChiChi_analysis_software/postprocess/binMaps/selectionStudy/%s/"%WP_str
			group_output_path = os.getenv('CMSSW_BASE') + "/src/SuuToChiChi_analysis_software/postprocess/superbinGroups/selectionStudy/%s/"%WP_str
			neighbor_output_path = os.getenv('CMSSW_BASE') + "/src/SuuToChiChi_analysis_software/postprocess/superbinNeighbors/selectionStudy/%s/"%WP_str
			output_plot_path     = os.getenv('CMSSW_BASE') + "/src/SuuToChiChi_analysis_software/postprocess/plots/statUncertaintyPlots/postMergedPlots/selectionStudy/%s/%s/"%(QCD_type,WP_str)

			print("bin map output dir is %s"%text_output_path)
			print("SB group map output dir is %s"%group_output_path)
			print("neighbor map output dir is %s"%neighbor_output_path)
			print("output plot output dir is %s"%output_plot_path)
			...
			if not os.path.exists(neighbor_output_path):
				os.makedirs(neighbor_output_path)
			if not os.path.exists(output_plot_path):
				os.makedirs(output_plot_path)


			if not dryRun:
				os.system('rm %s/%s_superbin_indices*.txt'%(text_output_path,QCD_type) )
				os.system('rm %s/%s_superbin_groups_*.txt'%(group_output_path,QCD_type) )
				os.system('rm %s/%s_superbin_neighbor_*.txt'%(neighbor_output_path,QCD_type) ) 

			for year in years:

				if not dryRun:  
					out_txt_file = open("%s/%s_superbin_indices_%s.txt"%(text_output_path,QCD_type,year),"a")
					superbin_group_txt_file = open("%s/%s_superbin_groups_%s.txt"%(group_output_path,QCD_type,year),"a")
					superbin_neighbor_txt_file = open("%s/%s_superbin_neighbors_%s.txt"%(neighbor_output_path,QCD_type,year),"a")
				...
				for iii,hist_name in enumerate(hist_names):
					...
					print("Creating maps for %s"%(year))

					# give histogram to constructor
					#try:
					testCase = combineHistBins(year, output_strs[iii], hist_name, debug,dryRun,runEos, useQCDHT_opt, WP_str, max_stat_uncert=0.175)
					...
					#		testCase = combineHistBins(year, output_strs[iii], debug,dryRun,runEos, WP_str, higher_stat_uncert_threshold)
					#	except:
					...
					bin_map_hist = ROOT.TH2F("bin_map_hist%s"%output_strs[iii], ("Superbin Map for %s (%s); disuperjet mass (GeV); avg. superjet mass (GeV)"%(year_str,technique_strs[iii])), testCase.n_bins_x,1250., 10000, testCase.n_bins_y, 500, 5000)  # 375 * 125
					stat_uncert_hist = ROOT.TH2F("stat_uncert_hist%s"%output_strs[iii], ("Superbin Statistical Uncertainty for %s (%s); disuperjet mass (GeV); avg. superjet mass (GeV)"%(year_str,technique_strs[iii])), testCase.n_bins_x,1250., 10000, testCase.n_bins_y, 500, 5000)  # 375 * 125
					merged_hist_count = ROOT.TH2F("merged_hist_count%s"%output_strs[iii], ("Unscaled Superbin Event Yields (post bin merging) for %s (%s); disuperjet mass (GeV); avg. superjet mass (GeV)"%(year_str,technique_strs[iii])), testCase.n_bins_x,1250., 10000, testCase.n_bins_y, 500, 5000)  # 375 * 125
					...
					if not dryRun:  
						out_txt_file.write("%s/%s/number of bins = %s/%s\n"%(year,hist_name, len(testCase.superbin_indices), testCase.superbin_indices))
						superbin_group_txt_file.write("%s/%s/number of superbin groups = %s/%s\n"%(year,hist_name,  len(testCase.superbin_groups), testCase.superbin_groups))
						superbin_neighbor_txt_file.write("%s/%s/number of superbin neighbors = %s/%s\n"%(year,hist_name, len(testCase.superbin_neighbors), testCase.superbin_neighbors))
					...
					c.SaveAs( output_plot_path +   "bin_map_%s%s_%s.png"%(WP_str, output_strs[iii],year)) 
					...
					write_cms_text(CMS_label_xpos=0.152, SIM_label_xpos=0.31,CMS_label_ypos = 0.92, SIM_label_ypos = 0.918, lumistuff_xpos=0.89, lumistuff_ypos=0.91, year = "", uses_data=False)
					c.SaveAs(output_plot_path +  "stat_uncert_%s%s_%s.png"%(WP_str, output_strs[iii],year)) 

					ROOT.gStyle.SetPalette(ROOT.kViridis)

					merged_hist_count.Draw("colz")
					write_cms_text(CMS_label_xpos=0.152, SIM_label_xpos=0.31,CMS_label_ypos = 0.92, SIM_label_ypos = 0.918, lumistuff_xpos=0.89, lumistuff_ypos=0.91, year = "", uses_data=False)
					c.SaveAs( output_plot_path +  "bin_counts_%s%s_%s.png"%(WP_str, output_strs[iii],year)) 
