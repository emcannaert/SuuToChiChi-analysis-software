...
TH1F* combineHistograms(const std::vector<std::string>& filePaths, std::string histName,std::string year, std::string hist_limit) 
{
    ...
    for (const auto& filePath : filePaths) 
    {
        ...
        TTree* tree_data_temp = (TTree*)file->Get("selectionStudier_nom/tree_nom");
...
{ 
    ...
    TTree* tree_QCD1000to1500 = (TTree*)QCD1000to1500_file->Get("selectionStudier_nom/tree_nom");
    TTree* tree_QCD1500to2000 = (TTree*)QCD1500to2000_file->Get("selectionStudier_nom/tree_nom");
    TTree* tree_QCD2000toInf = (TTree*)QCD2000toInf_file->Get("selectionStudier_nom/tree_nom");


    // Get the TTrees from the files
    TTree* tree_TTToHadronic = (TTree*)TTToHadronic_file->Get("selectionStudier_nom/tree_nom");
    TTree* tree_TTToSemiLeptonic = (TTree*)TTToSemiLeptonic_file->Get("selectionStudier_nom/tree_nom");
    TTree* tree_TTToLeptonic = (TTree*)TTToLeptonic_file->Get("selectionStudier_nom/tree_nom");
    ...
    c->SaveAs(Form("plots/dataMC/initialSelection/%s_dataMC_%s.png", histName.c_str(), year.c_str()));
 
}
void plot_initial_selection_dataMC()
{
    ...
    for (size_t  iii = 0; iii < years.size(); iii++) 
    {
        ...
        std::string eosPath = "root://cmseos.fnal.gov//store/user/ecannaer/selectionStudy_combinedROOT/";
