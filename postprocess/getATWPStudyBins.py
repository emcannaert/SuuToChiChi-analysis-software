...
class combineHistBins:
	...
	def sort_bins_by_descending_event_yield(self): 
		...
		WP_folder = self.WP[2:]
		plt.savefig('plots/statUncertaintyPlots/postMergedPlots/ATWPStudy/%s/post_merge_superbin_yields_%s_%s%s_%s.png'%(WP_folder, self.year,self.technique_str,self.region,self.WP))
		plt.close()
...
if __name__=="__main__":
	...
	regions = ["AT1b", "AT0b"]   # "AT0tb", "AT1tb", "SB1b", "SB0b"
	...
	for WP in WPs:
		...
		text_output_path  = os.getenv('CMSSW_BASE') + "/src/postprocess/binMaps/ATWPStudy/%s"%WP_str
		group_output_path = os.getenv('CMSSW_BASE') + "/src/postprocess/superbinGroups/ATWPStudy/%s"%WP_str
		neighbor_output_path = os.getenv('CMSSW_BASE') + "/src/postprocess/superbinNeighbors/ATWPStudy/%s"%WP_str
		...
		for region in regions:
			for year in years:
				for iii,hist_name in enumerate(hist_names):
					...
					WP_str_use = "AT"+  WP_str
					#try:
					testCase = combineHistBins(year, region, output_strs[iii], debug,dryRun,runEos, WP_str_use, max_stat_uncert=0.175)
					...
					"""except: 
						...
						try: 
							...
							testCase = combineHistBins(year, region, output_strs[iii], debug,dryRun,runEos, WP_str_use, higher_stat_uncert_threshold)
						except:
					...
					c.SaveAs( os.getenv('CMSSW_BASE')+  "/src/postprocess/plots/statUncertaintyPlots/postMergedPlots/ATWPStudy/%s/bin_map_%s%s_%s.png"%(WP_str, output_strs[iii],region,year)) 
					...
					write_cms_text(CMS_label_xpos=0.152, SIM_label_xpos=0.31,CMS_label_ypos = 0.92, SIM_label_ypos = 0.918, lumistuff_xpos=0.89, lumistuff_ypos=0.91, year = "", uses_data=False)
					c.SaveAs(os.getenv('CMSSW_BASE')+  "/src/postprocess/plots/statUncertaintyPlots/postMergedPlots/ATWPStudy/%s/stat_uncert_%s%s_%s.png"%(WP_str, output_strs[iii],region,year)) 

					ROOT.gStyle.SetPalette(ROOT.kViridis)

					merged_hist_count.Draw("colz")
					write_cms_text(CMS_label_xpos=0.152, SIM_label_xpos=0.31,CMS_label_ypos = 0.92, SIM_label_ypos = 0.918, lumistuff_xpos=0.89, lumistuff_ypos=0.91, year = "", uses_data=False)
					c.SaveAs(os.getenv('CMSSW_BASE')+  "/src/postprocess/plots/statUncertaintyPlots/postMergedPlots/ATWPStudy/%s/bin_counts_%s%s_%s.png"%(WP_str, output_strs[iii],region,year)) 
