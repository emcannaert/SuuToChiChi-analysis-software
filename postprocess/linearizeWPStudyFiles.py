...
class linearized_plot:
	...
	def __init__(self, year, mass_point, technique_str, all_BR_hists, useMask=False, createDummyChannel=False, run_from_eos = False, debug=False, WP=None):
		...
		self.index_file_home	 = os.getenv('CMSSW_BASE') + "/src/postprocess/binMaps/WPStudy/WP%s/"%WP
		self.output_file_home	= os.getenv('CMSSW_BASE') + "/src/postprocess/WP_study/finalCombineFilesNewStats/WP%s/"%WP
		self.final_plot_home	 = os.getenv('CMSSW_BASE') + "/src/postprocess/plots/WP_study/finalCombinePlots/WP%s/"%WP
