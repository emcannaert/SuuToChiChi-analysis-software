...
class linearized_plot:
	...
	def __init__(self, year,mass_point, technique_str):
		...
		ST_s_channel_hadrons
		ST_s_channel_leptons
		ST_tW_antitop
		ST_tW_top
		"""
		self.signal_hist_SR = []
		...
		for systematic in self.systematics:
			...
			### create combined yield histograms:
			"""
				all_combined_hists_SR
				all_combined_hists_CR
				all_combined_hists_AT1b
				all_combined_hists_AT0b
			"""
		...
		for iii,systematic_ in enumerate(self.systematic_names):   ### this was originally extending the
			...
			self.TTbar_linear_SR.append(self.linearize_plot([],"TTbar","SR",systematic + sample_type + year_str, False,"TTbar", [ self.TTJets1200to2500_hist_SR[iii], self.TTJets2500toInf_hist_SR[iii]] ))
			self.TTbar_linear_CR.append(self.linearize_plot([],"TTbar","CR",systematic + sample_type + year_str, False,"TTbar", [ self.TTJets1200to2500_hist_CR[iii], self.TTJets2500toInf_hist_CR[iii]]))
			self.TTbar_linear_AT0b.append(self.linearize_plot([],"TTbar","AT0b",systematic + sample_type + year_str, False,"TTbar", [ self.TTJets1200to2500_hist_AT0b[iii], self.TTJets2500toInf_hist_AT0b[iii]]))
			self.TTbar_linear_AT1b.append(self.linearize_plot([],"TTbar","AT1b",systematic + sample_type + year_str, False,"TTbar", [ self.TTJets1200to2500_hist_AT1b[iii], self.TTJets2500toInf_hist_AT1b[iii]]))
			...
			self.ST_linear_AT1b.append(self.linearize_plot([],"ST","AT1b",systematic + sample_type + year_str, False,"ST", [ self.ST_t_channel_top_hist_AT1b[iii], self.ST_t_channel_antitop_hist_AT1b[iii], self.ST_s_channel_hadrons_hist_AT1b[iii], self.ST_s_channel_leptons_hist_AT1b[iii], self.ST_tW_antitop_hist_AT1b[iii],self.ST_tW_top_hist_AT1b[iii]] ))
				...
				#### THESE NEED TO BE FIXED TO USE THE CORRECT "self.ST_t_channel_top_hist_SR[iii]" --> self.ST_t_channel_top_hist_SB1b[iii]
			#if self.doSideband: self.ST_linear_SB1b.append(self.linearize_plot([],"ST","SB1b",systematic + sample_type + year_str, "ST", [ self.ST_t_channel_top_hist_[iii], self.ST_t_channel_antitop_hist_SR[iii], self.ST_s_channel_hadrons_hist_SR[iii], self.ST_s_channel_leptons_hist_SR[iii], self.ST_tW_antitop_hist_SR[iii],self.ST_tW_top_hist_SR[iii]] ))
			#if self.doSideband: self.ST_linear_SB0b.append(self.linearize_plot([],"ST","SB0b",systematic + sample_type + year_str, "ST", [ self.ST_t_channel_top_hist_SR[iii], self.ST_t_channel_antitop_hist_SR[iii], self.ST_s_channel_hadrons_hist_SR[iii], self.ST_s_channel_leptons_hist_SR[iii], self.ST_tW_antitop_hist_SR[iii],self.ST_tW_top_hist_SR[iii]] ))
	...
	def add_stat_uncertainties(self ):
		...
		### now calculate the stat uncertainty and add this branch to the final histogram ____ 
		...
		for iii in range(1,self.QCD_linear_CR[0][0].GetNbinsX()+1):
			...
			QCD_bin_nom_uncert    = self.QCD_linear_CR[0][0].GetBinError(iii)
		...
		for iii in range(1,self.QCD_linear_AT1b[0][0].GetNbinsX()+1):
			...
			QCD_bin_nom_uncert    = self.QCD_linear_AT1b[0][0].GetBinError(iii)
		...
		for iii in range(1,self.QCD_linear_AT0b[0][0].GetNbinsX()+1):
			...
			QCD_bin_nom_uncert    = self.QCD_linear_AT0b[0][0].GetBinError(iii)
	...
	def load_QCD_hists(self,region,systematic, forStats = False, hist_type = ""):
		...
		for sys_str in sys_updown:
			...
			elif hist_type == "QCDMC2000toInf":  all_combined_QCD_hist.append( TH2_hist_2000toInf   )  #### THESE ARE UNSCALED!!!
	...
	def load_ttbar_hist(self,region,systematic, forStats = False, hist_type = ""):
		...
		for sys_str in sys_updown:
			...
			elif hist_type == "TTJets2500toInf": all_combined_TTbar_hist.append( TH2_hist_TTJetsMCHT2500toInf  )
	...
	def load_signal_hist(self,region,systematic, forStats=False, hist_type = ""):
		...
		for sys_str in sys_updown:
			...
			if hist_type == "":
				...
				if not signal_hist: signal_hist = ROOT.TH2F("h_MSJ_mass_vs_MdSJ_%s"%(region),"Superjet mass vs diSuperjet mass (%s) (%s) (%s); diSuperjet mass [GeV];superjet mass"%(region, hist_type,self.technique_str ), 22,1250., 10000, 20, 500, 5000) # 375 * 125
	...
	def linearize_plot(self,_hist,BR_type,region,systematic, forStats=False, hist_type="", split_up_hists_for_systematic = []): 
		...
		for iii,sys_str in enumerate(sys_updown):
			...
			if hist_type == "QCD":
				...
				linear_plot_QCD1000to1500 = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "QCDMC1000to1500",region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_QCD1500to2000 = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "QCDMC1500to2000",region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_QCD2000toInf = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%(  "QCDMC2000toInf",region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				...
				### need to lineraize the histograms separately and THEN add them to linear_plot
				linear_plot_TTJets1200to2500 = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "TTJetsMCHT1200to2500",region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_TTJets2500toInf  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "TTJetsMCHT2500toInf" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				...
				### need to lineraize the histograms separately and THEN add them to linear_plot
				linear_plot_ST_t_channel_top_5f = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "ST_t_channel_top_5f",region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_ST_t_channel_antitop_5f  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "ST_t_channel_antitop_5f" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_ST_s_channel_4f_hadrons  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "ST_s_channel_4f_hadrons" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_ST_s_channel_4f_leptons  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s) ; bin; Events / bin"%( "ST_s_channel_4f_leptons" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_ST_tW_antitop_5f  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "ST_tW_antitop_5f" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_ST_tW_top_5f  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "ST_tW_top_5f" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				...
				### need to lineraize the histograms separately and THEN add them to linear_plot
				linear_plot_WBWB = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "WBWB",region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_HTHT = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "HTHT" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_ZTZT  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "ZTZT" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_WBHT  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s) ; bin; Events / bin"%( "WBHT" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_WBZT = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "WBZT" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
				linear_plot_HTZT  = ROOT.TH1D("%s%s"%(BR_type,sys_str),"linearized %s in the %s (%s) (%s); bin; Events / bin"%( "HTZT" ,region,year, " ".join(use_sys.split("_"))),linear_plot_size,0,linear_plot_size)
