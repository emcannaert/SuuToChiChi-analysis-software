...
def make_BR_sig_superimposed( year, mass_point,tagging_type, tagging_str, hist_name, region ):
	...
	QCDMC1000to1500_file= ROOT.TFile(QCDMC1000to1500_filename, "READ")
	QCDMC1500to2000_file = ROOT.TFile(QCDMC1500to2000_filename, "READ")
	QCDMC2000toInf_file  = ROOT.TFile(QCDMC2000toInf_filename, "READ")

	TTJetsMCHT1200to2500_file  = ROOT.TFile(TTJetsMCHT1200to2500_filename, "READ")
	TTJetsMCHT2500toInf_file   = ROOT.TFile(TTJetsMCHT2500toInf_filename, "READ")

	ST_t_channel_top_5f_file  	   = ROOT.TFile(ST_t_channel_top_5f_filename, "READ")
	ST_t_channel_antitop_5f_file   = ROOT.TFile(ST_t_channel_antitop_5f_filename, "READ")
	ST_s_channel_4f_hadrons_file   = ROOT.TFile(ST_s_channel_4f_hadrons_filename, "READ")
	ST_s_channel_4f_leptons_file   = ROOT.TFile(ST_s_channel_4f_leptons_filename, "READ")
	ST_tW_antitop_5f_file  		= ROOT.TFile(ST_tW_antitop_5f_filename, "READ")
	ST_tW_top_5f_file   		= ROOT.TFile(ST_tW_top_5f_filename, "READ")
	...
	QCD_combined.SetFillColor(ROOT.kRed)
	...
	TTbar_combined.SetFillColor(ROOT.kYellow)
	...
	ST_combined.SetFillColor(ROOT.kGreen)
...
def create_superimposed_SJ_mass_plot_combined(year, MSuu, tagging_type, tagging_str):
	...
	for iii in range(0,len(decays)):      ### first index is the decay
		...
		for jjj,Mchi in enumerate(chi_masses):			  ### second index is the chi mass: [1, 1p5, 2, 2p5, 3]
			...
			inFileName2 = inFile_dir + "%s_%s_%s_processed.root"%(mass_point,decays[iii],year)
			file_list[iii].append(ROOT.TFile(inFileName2,"r"))
...
def make_combined_plots( year, mass_point,tagging_type, tagging_str ):
	...
	h_MSJ_mass_vs_MdSJ_SR.SetTitle("SuuToChiChi signal avg. SJ mass vs diSuperjet mass (combined) (%s) (%s) (SR) (%s)"%(mass_point,year_str,tagging_str))
	h_MSJ_mass_vs_MdSJ_CR.SetTitle("SuuToChiChi signal avg. SJ mass vs diSuperjet mass (combined) (%s) (%s) (CR) (%s)"%(mass_point,year_str,tagging_str))
	h_MSJ_mass_vs_MdSJ_AT1b.SetTitle("SuuToChiChi signal tagged SJ mass vs diSuperjet mass (combined) (%s) (%s) (AT1b) (%s)"%(mass_point,year_str,tagging_str))
	h_MSJ_mass_vs_MdSJ_AT0b.SetTitle("SuuToChiChi signal tagged SJ mass vs diSuperjet mass (combined) (%s) (%s) (AT0b) (%s)"%(mass_point,year_str,tagging_str))
